...
class BuildFbm{
	...
	public:
		BuildFbm(double size, double H, double seed);
...
BuildFbm::BuildFbm(double size, double H, double seed){
	crtnmap = new terr::CPointsMap();
	libfbm::zvec dim(2);
  	dim[0] = 64;
  	dim[1] = 64;
  	//MyContext ctx(dim);
	libfbm::FBMSteinContext ctx(H, 2, (int)size);  //Hurst exponent H ctx(H, *,*) controls smoothness of the terrain with larger values
