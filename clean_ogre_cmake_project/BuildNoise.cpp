	...
	for (int j = 0; j < size; j++){
		...
		for (int k = 0; k < texscale; k++){
			...
			for (int l = 0; l < texscale; l++){
				...
				terr::Coordpair * coordpair = new terr::Coordpair((double)i+subinck,(double)j+subincl);
                		double x = (((double)i)+subinck) * 513.0f/(512.0f);
				double y = (((double)j)+subincl) * 513.0f/(512.0f);
...
terr::CPointsMap   BLI_gNoisebuild(double size,double scale, double texscale, int hard, int noisebasis, bool repeat){
   terr::CPointsMap * points = new terr::CPointsMap();
   double maxval = (double)0.0f, minval = (double)100000000.0f; double val;
   double sizem1 = size-1.0f;
   double tr = (sizem1)/scale;  double w = (sizem1*sizem1)/(scale*scale);
   for (int i = 0; i < size; i++){
	for (int j = 0; j < size; j++){
		//terr::Coordpair * coordpair = new terr::Coordpair(i,j);
		for (int k = 0; k < texscale; k++){
			...
			for (int l = 0; l < texscale; l++){
				double subincl = ((double) l)/texscale;
				terr::Coordpair * coordpair = new terr::Coordpair((double)i+subinck,(double)j+subincl);
				double x = (((double)i)+subinck);/// scale;
				double y = (((double)j)+subincl);/// scale;
				double z = 0.0f;
				Vector  point; point.x = x; point.y = y; point.z = z;
               			//float mg_fBm(float x, float y, float z, float H, float lacunarity, float octaves, int noisebasis);
				if(repeat){
					double xp = x-sizem1;  double yp = y - sizem1;
		...
		for (int k = 0; k < texscale; k++){
			...
			for (int l = 0; l < texscale; l++){
				...
				Vector  point; point.x = x; point.y = y; point.z = z;
                		//float mg_fBm(float x, float y, float z, float H, float lacunarity, float octaves, int noisebasis);
				if(repeat){
					double xp = x-sizem1/scale;  double yp = y - sizem1/scale;
