...
class BuildVoronoi{
	...
	private:
		...
		vor::CPointsMap * buildPoints(vor::Cells * cells);
};
...
//*/
	...
	for (int j = 0; j<size; j++){
		for (int k = 0; k<size; k++){
			...
			double mult = w/size; int jmn = j*mult; int kmn = k*mult; double jmm = j*mult, kmm = k*mult; 
			...
			if ((*pointsmap).find((*coordpair)) != (*pointsmap).end()){
				...
				Ogre::ColourValue col = Ogre::ColourValue(1.0f,1.0f,1.0f);
				fill->setPixl((size_t)j, (size_t)k, col);
			}
...
vor::CPointsMap * BuildVoronoi::buildPoints(vor::Cells * cells){
		...
		*/
		...
		while(y >= ymin){
			...
			while (l <= dmax){
				...
				dist = pow(pow(sitepos->y-m,2.0f) + pow(sitepos->x-l, 2.0f),.5f);
				(*pointsmap)[ipos] = dist;
				vor::Coordpair * coordpair = new vor::Coordpair(ipos->x, ipos->y);
				(*cpointsmap)[(*coordpair)] = dist;
				if (maxval < dist){
					maxval = dist;
				}
