...
class LoadHeightMap{
	public:
		...
		LoadHeightMap(Ogre::Terrain* mterrain, Ogre::Rectangle2D* mMiniScreen, 
...
LoadHeightMap::LoadHeightMap(Ogre::Terrain* mterrain, Ogre::Rectangle2D* mMiniScreen, 
			     float maxheightscale, terr::CPointsMap * heightmapvalues, double waterheight, 
...
void LoadHeightMap::updateBlendMap(double alpha, int layerid){
    Ogre::TerrainLayerBlendMap* blendMap0 = cmterrain->getLayerBlendMap(layerid);
    float* pBlend0 = blendMap0->getBlendPointer();
    for (Ogre::uint16 y = 0; y < cmterrain->getLayerBlendMapSize(); ++y)
    {
        for (Ogre::uint16 x = 0; x < cmterrain->getLayerBlendMapSize(); ++x)
        {
            Ogre::Real tx, ty;
 
            blendMap0->convertImageToTerrainSpace(x, y, &tx, &ty);
            Ogre::Real height = cmterrain->getHeightAtTerrainPosition(tx, ty);
