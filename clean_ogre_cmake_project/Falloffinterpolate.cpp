#ifndef __Falloffinterpolate_CPP
#define __Falloffinterpolate_CPP
...
class Falloffinterpolate{
	public:
		...
		Falloffinterpolate(double itparam, double rfalloff, Ogre::Vector4 icoeff, double tramount);
		...
		double getTPoint(void);
		Ogre::Vector3 getIpoint(void);
		vector<Ogre::Vector3> getIpoints(void);
	private:
		double computetparam(void);  //normalized distance measure from ccpoint to cpoint.
		Ogre::Vector3 computeinterp(void);
		vector<Ogre::Vector3> cfalloffSelection;
		//std::vector<Ogre::Real> Ipoints;
		vector<Ogre::Vector3> Ipoints;

};

Falloffinterpolate::Falloffinterpolate(double itparam, double rfalloff, Ogre::Vector4 icoeff, double tramount){
	cpoint = Ogre::Vector3(0, 0, 0);
//	ccpoint = clpoint;
	tparam = itparam;
	cicoeff = icoeff;
	crfalloff = rfalloff;
...
double Falloffinterpolate::getTPoint(){
	return (double) cipoint.y;
}
