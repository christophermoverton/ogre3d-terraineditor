...
class FreqAmpOctEventReg{
	public:
		...
		void updateCWS(const CEGUI::EventArgs &e);  //Continent Weight slider
		void updateCWE(const CEGUI::EventArgs &e);  //Continent Weight edit box
	...
	private:
		...
		CEGUI::Window *ContinentWslider;
		CEGUI::Window *ContinentWEditbox;
		...
		TextureConfigs ptexconfigs;  //previous configuration settings state
		bool tconfignpchng;  //texture noise configurations changed
		bool tconfigntschng;  //texture settings configurations changed
		bool nupdate;  //check for last noise update
		void addconfig(int nameID);
		void updateConfig(void);
		void loadConfig(void);
		void updateNoiseTypesettings(void);
		void checkTexConfigState(void);
		double findMax(double val1, double val2, double val3, double val4);
		void getHeightdata(ConfSets tc, int i, bool repeat, bool Continent, terr::CPointsMaps *vnoisevalsm);
		void combineContRegHeightdata(terr::CPointsMap * contdat, terr:: CPointsMap * localhdat, 
						   terr::CPointsMaps *combreturnvals);
...
FreqAmpOctEventReg::FreqAmpOctEventReg(Ogre::TerrainGroup* terraingroup, Ogre::Camera* Camera, Ogre::Rectangle2D* mMiniScreen){
	...
	nupdate = false;
	tconfignpchng = false;  //texture noise configurations changed
	tconfigntschng = false; 
	...
	//ScrollablePane = tcnewWindow->getChild("ScrollablePane");
	...
	ContUseCheckbox = TerrainTexturesWindow->getChild("ContUseCheckbox");
	UpdateTexButton = TerrainTexturesWindow->getChild("UpdateTexButton");
	//CEGUI::ScrollablePane * scrollablepane = static_cast<CEGUI::ScrollablePane*>(ScrollablePane);
	//ScrollablePane->addChild(TerrainTexturesWindow);
	...
	//tabcontrol->addTab(ScrollablePane);
	...
	ContinentWslider = TerrainSettingsWindow->getChild("ContinentWeightSlider");
	ContinentWEditbox = TerrainSettingsWindow->getChild("ContinentWeightEditbox");
...
//	Terrainbox->addItem(listboxitem2);
	...
	checkbox->setSelected( false );
	checkbox = static_cast<CEGUI::ToggleButton*>(ContUseCheckbox);
	checkbox->setSelected( false );
	...
	ContinentWslider->subscribeEvent(CEGUI::Slider::EventValueChanged, CEGUI::Event::Subscriber(&FreqAmpOctEventReg::updateCWS, this));
	ContinentWEditbox->subscribeEvent(CEGUI::Editbox::EventTextAccepted, CEGUI::Event::Subscriber(&FreqAmpOctEventReg::updateCWE, this));
...
void FreqAmpOctEventReg::checkTexConfigState(){
	if (nupdate) {
		tconfignpchng = false;
		tconfigntschng = false;
	}
	
	for (TextureConfigs::iterator i = texconfigs.begin(); i != texconfigs.end(); i++){
		std::string name = (*i).first;
		
//		bool tconfignpchng;  //texture noise configurations changed
//		bool tconfigntschng;  //texture settings configurations changed
		Configs conf = i->second;
		for (Configs::iterator j = conf.begin(); j != conf.end(); j++){
			int noiseid = j->first;
			if (noiseid == 3) {break;}
			bool brnext = false;
			ConfSets c = j->second;
			for (ConfSets::iterator k = c.begin(); k != c.end(); k++){
				std::string state = k->first;
				if (ptexconfigs[name][noiseid][state] != k->second){
					tconfignpchng = true;
					brnext = true;
					nupdate = false; //configurations changed relative to last update
					break;
				}
			}
			if (brnext) {break;}
		...
		for (ConfSets::iterator k = c.begin(); k != c.end(); k++){
			std::string state = k->first;
			if (ptexconfigs[name][3][state] != k->second){
				tconfigntschng = true;
				nupdate = false;
				break;
			}
...
void FreqAmpOctEventReg::updateCWE(const CEGUI::EventArgs &e){
	
	CEGUI::Slider* CWSlider = static_cast<CEGUI::Slider*>(ContinentWslider);
	//float val = SSlider->getCurrentValue();
	CEGUI::Editbox* CWEbox = static_cast<CEGUI::Editbox*>(ContinentWEditbox);
	float val = atof(CWEbox->getText().c_str());
	std::ostringstream ss5;
	ss5<<val;
	CWSlider->setCurrentValue(val);
}

void FreqAmpOctEventReg::updateCWS(const CEGUI::EventArgs &e){
	
	CEGUI::Slider* CWSlider = static_cast<CEGUI::Slider*>(ContinentWslider);
	float val = CWSlider->getCurrentValue();
	CEGUI::Editbox* CWEbox = static_cast<CEGUI::Editbox*>(ContinentWEditbox);
	std::ostringstream ss5;
	ss5<<val;
	CWEbox->setText(ss5.str());
}
...
double FreqAmpOctEventReg::findMax(double val1, double val2, double val3, double val4){
...
void FreqAmpOctEventReg::combineContRegHeightdata(terr::CPointsMap * contdat, terr:: CPointsMap * localhdat, 
						   terr::CPointsMaps * combreturnvals){
	terr::CPointsMap * map1 = new terr::CPointsMap();
	terr::CPointsMap * map2 = new terr::CPointsMap();
	terr::CPointsMap * map3 = new terr::CPointsMap();
	terr::CPointsMap * map4 = new terr::CPointsMap();
	double maxval = (double) 0.0f;  //all heightdata normalized within range [0,1] preprocessing wise
	CEGUI::Slider* CWSlider = static_cast<CEGUI::Slider*>(ContinentWslider);
	float wval = CWSlider->getCurrentValue();
	for (int i = 0; i < 513; i++){
	    for (int j = 0; j < 513; j++){
		terr::Coordpair * coord1 = new terr::Coordpair(i,j);
		terr::Coordpair * coord4 = new terr::Coordpair(i+513,j);
		terr::Coordpair * coord2 = new terr::Coordpair(i,j+513);
		terr::Coordpair * coord3 = new terr::Coordpair(i+513,j+513);
		double locheight = (*localhdat)[*coord1];
		double contheight1 = (*contdat)[*coord1];
		double contheight2 = (*contdat)[*coord2];
		double contheight3 = (*contdat)[*coord3];
		double contheight4 = (*contdat)[*coord4];
		(*map1)[*coord1] = (1.0f - wval)*locheight + wval*contheight1;//lerp(locheight,contheight1,(double)1.0f);
		(*map2)[*coord1] = (1.0f - wval)*locheight + wval*contheight2;//lerp(locheight,contheight2,(double)1.0f);
		(*map3)[*coord1] = (1.0f - wval)*locheight + wval*contheight3;//lerp(locheight,contheight3,(double)1.0f);
		(*map4)[*coord1] = (1.0f - wval)*locheight + wval*contheight4;//lerp(locheight,contheight4,(double)1.0f);
		double max = findMax((*map1)[*coord1], (*map2)[*coord1], (*map3)[*coord1], (*map4)[*coord1]);
		if (max > maxval){
		   maxval = max;
		}
	    }
	...
	(*combreturnvals)[0] = *map1; (*combreturnvals)[1] = *map2;
	(*combreturnvals)[2] = *map3; (*combreturnvals)[3] = *map4;
}

void FreqAmpOctEventReg::getHeightdata(ConfSets tc, int i, bool repeat, bool Continent, terr::CPointsMaps * vnoisevalsm){
	double res = 513.0f;
	if (Continent){res = 1026.0f;}
	if ((int)tc["NoiseType"] == 0){

		(*vnoisevalsm)[i] = pfbmbuild (res, tc["Scale"], 1.0f, tc["Frequency"]/tc["Freqdiv"], tc["Amplitude"], tc["Octaves"],
                                                       tc["Gain"], repeat);
					
	}
	else if ((int)tc["NoiseType"]==1){
		(*vnoisevalsm)[i] =  BLI_gNoisebuild(res, tc["Scale"], 1.0f,tc["Sharp"], tc["NoiseBasis"], repeat);
	}
	else if ((int)tc["NoiseType"]==2){

		BuildSimplex * tests = new BuildSimplex(res, tc["Frequency"]/tc["Freqdiv"], tc["Amplitude"]);  //perlin derivative

		//(*tnoisevalsm)[i] = tests->getHeightMap();
		(*vnoisevalsm)[i] = tests->getHeightMap2();
	}
	else if ((int)tc["NoiseType"]==3){
		BuildFbm * testf = new BuildFbm(res, tc["Frequency"], tc["Amplitude"]);  
		(*vnoisevalsm)[i] = testf->getHeightMap();
	}
	else if ((int)tc["NoiseType"]==4){
		DiamondSquare * testd = new DiamondSquare(res); 
		(*vnoisevalsm)[i] = testd->getHeightMap();
	}
	else if ((int)tc["NoiseType"]==5){
                (*vnoisevalsm)[i] = mg_MultiFractalbuild(res, tc["Scale"], 1.0f,tc["Depth"], tc["Frequency"], tc["Octaves"],
								 tc["NoiseBasis"], repeat);
		
	}
	else if ((int)tc["NoiseType"]==6){
                (*vnoisevalsm)[i] = mg_HeteroTerrainbuild(res, tc["Scale"], 1.0f,tc["Depth"], tc["Frequency"], 
						tc["Octaves"], tc["Amplitude"], tc["NoiseBasis"], repeat);
	}
	else if ((int)tc["NoiseType"]==7){
		(*vnoisevalsm)[i] = mg_HybridMultiFractalbuild(res,tc["Scale"], 1.0f,tc["Depth"], tc["Frequency"], 
						tc["Octaves"], tc["Amplitude"], tc["Gain"], tc["NoiseBasis"], repeat); 
	}
	else if ((int)tc["NoiseType"]==8){
		(*vnoisevalsm)[i] =  mg_RidgedMultiFractalbuild(res,tc["Scale"], 1.0f, tc["Depth"], tc["Frequency"], 
						tc["Octaves"], tc["Amplitude"], tc["Gain"], tc["NoiseBasis"], repeat); 
	}
	else if ((int)tc["NoiseType"]==9){
		(*vnoisevalsm)[i] = pmarblebuild (res,tc["Scale"]/265.5f, 1.0f, tc["Frequency"]/tc["Freqdiv"], tc["Amplitude"], 
...
*/	
	...
	config = texconfigs[TextureSelection];
	Configs configs2; 
	bool terraincont = false;  //selecting terrain/continent
	CEGUI::ToggleButton* checkbox = static_cast<CEGUI::ToggleButton*>(ContUseCheckbox);
	bool valueCheckbox = checkbox->isSelected();
	if (TextureSelection == "Continent"){
		config = texconfigs["Terrain"];
		configs2 = texconfigs[TextureSelection];
		if (valueCheckbox){
		    terraincont = true;
		}
	}
	else if (TextureSelection == "Terrain"){
		configs2 = texconfigs["Continent"];
		if (valueCheckbox){
		    terraincont = true;
		}
	}
	...
	for (miter3 = config.begin(); miter3 != config.end(); miter3++){
		...
		getHeightdata(tc, i, repeat, false, vnoisevalsm);
...
//	vector<vector<vector<double> > > heightmapvalues = test->getNoisevalues();
	...
	//if (valueCheckbox and ((TextureSelection == "Continent") or (TextureSelection == "Terrain"))){ terraincont = true;}	
	terr::CPointsMaps * rcheights = new terr::CPointsMaps();
        terr::CPointsMaps * contvnoisevalsm = new terr::CPointsMaps();
	if (terraincont){
		int i = 0;
		Configs:: iterator miter4;
		vector<float> weights2(config.size());
		for (miter4 = configs2.begin(); miter4 != configs2.end(); miter4++){
			ConfSets tcont = miter4->second;	
			if (miter4->first == 3) {break;}
			ss5 << "Starting noise gen loop!!!!!!!" << "\n";
			ss5 << "NoiseType: " << miter3->second["NoiseType"] << "\n";
			tlog->logMessage(ss5.str());
			ss5.str(std::string());	
			...
			if (tcont["Freqdiv"] == 0){tcont["Freqdiv"] = 1.0f;}
			bool repeat = false;
			if (configs2[3]["Repeat"] == 1.0f){repeat = true;}
			
			getHeightdata(tcont, i, repeat, true, contvnoisevalsm);
			weights2[i] = tcont["Weight"];
			i++;
		}
		CEGUI::Combobox* combobox = static_cast<CEGUI::Combobox*>(FilterTypebox);
		CEGUI::ListboxItem* litem = combobox->getSelectedItem();
		...
		Combiner* cmb = new Combiner(nameid);
		bool mrepeat = false;
		if (configs2[3]["Mirror"]==1.0f){mrepeat = true;}
		terr::CPointsMap * contheightmapvaluesm = cmb->Combine(contvnoisevalsm,weights2,mrepeat);
		//terr::CPointsMaps * rcheights = new terr::CPointsMaps();
		combineContRegHeightdata(contheightmapvaluesm, cheightmapvaluesm, rcheights);
	}
	CEGUI::Slider* WHSlider = static_cast<CEGUI::Slider*>(WaterHslider);
	float whval = WHSlider->getCurrentValue();
	CEGUI::Slider* MHSlider = static_cast<CEGUI::Slider*>(MountainHslider);
	float mhval = MHSlider->getCurrentValue();
	TerTexsets * ttexsets = new TerTexsets();
	if (terraincont){
	   for (int i = 0; i < 4; i++){
		if (i != 0){
			ss5 << "Terrain" << i+1;
		...
		texsets->name = ss5.str();
		if (TextureSelection == "Terrain"){texsets->layerid = 1;}
		else{texsets->layerid = 2;}
		ss5.str(std::string());
		ttexsets->push_back(*texsets);
				
	   }
	   new LoadHeightMap(cTerrainGroup, cmMiniScreen,513.0f*1.0f, rcheights, whval, mhval,2.0f, true, *ttexsets);
        }
	else{
	    TerrainTexturesSettings * texsets = new TerrainTexturesSettings();
	    texsets->size = config[3]["TextureSize"];
	    ...
	    texsets->terraintextures = 4;  //volume of base terrain textures
	    if (TextureSelection == "Terrain"){texsets->layerid = 1;}
	    else{texsets->layerid = 2;}
	...
	nupdate = true;
	delete vnoisevalsm;
	delete contvnoisevalsm;
	delete cheightmapvaluesm;
	delete rcheights;
...
void FreqAmpOctEventReg::updateTGS(const CEGUI::EventArgs &e){
	
	CEGUI::Slider* TGSlider = static_cast<CEGUI::Slider*>(Texgradslider);
	float val = TGSlider->getCurrentValue();
	CEGUI::Editbox* TGbox = static_cast<CEGUI::Editbox*>(TexGradEditbox);
	std::ostringstream ss5;
	ss5<<val;
	TGbox->setText(ss5.str());
}

void FreqAmpOctEventReg::updateTGB(const CEGUI::EventArgs &e){
