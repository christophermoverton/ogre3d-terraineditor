...
void FreqAmpOctEventReg::updateConfig(){
	...
	CEGUI::Combobox* combobox = static_cast<CEGUI::Combobox*>(Terrainbox);
	ncombobox = static_cast<CEGUI::Combobox*>(NoiseTypebox);
...
void FreqAmpOctEventReg::loadConfig(){
	...
	ncombobox->setItemSelectState 	(itemCombobox,true);
...
*/	
	...
	for (int i = 0; i < config.size(); i++){
		...
		map<string, float> tc = config[i+1];
		if ((int)tc["NoiseType"] == 1){
			...
			tnoisevals[i] = test->getHeightMapradial(double tramount);
		}
		weights[i] = tc["Weight"];
	}
	CEGUI::Combobox* combobox = static_cast<CEGUI::Combobox*>(FilterTypebox);
