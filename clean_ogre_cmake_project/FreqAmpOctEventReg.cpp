...
void FreqAmpOctEventReg::loadConfig(){
	...
	float fval, aval, oval, gval, sval, fdval, mval, wval, nval, dval;
	...
	for (miter3 = config.begin(); miter3 != config.end(); miter3++){
		...
		if ((int)tc["NoiseType"] == 0){
			...
			ss5.str(std::string());
                        ...
                        //pmarblebuild (double size, double scale, double frequency, double amplitude, double octaves, double gain)
